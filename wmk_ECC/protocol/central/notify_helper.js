...
function send_notify(msg) {
    notification.emit(Emitter.notify_service, 'notify', msg);
}
function on_send_notify(handler) {
    notification.on(Emitter.notify_service, 'notify', handler);
}
...
function process_server_notify(msg) {
    notification.emit(Emitter.notify_service, 's_notify', msg);
}
function on_server_notify(handler) {
    notification.on(Emitter.notify_service, 's_notify', handler);
}

function process_client_notify(client, msg) {
    notification.emit(Emitter.notify_service, 'c_notify', client, msg);
}
function on_client_notify(handler) {
    notification.on(Emitter.notify_service, 'c_notify', handler);
}
