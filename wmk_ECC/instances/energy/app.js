...
function init_central_service(callback) {
    ...
    server_manager.init({host: config.central.host, port: config.central.port}, {host: config.energy.host, port: config.energy.nport, type: svrType, notifies: [
...
function init_modules(callback) {
    var energy = require('../../modules/energy');
    energy.init(function (err) {
        if(err) return callback(err);

        var http = require('../../interface');
        http.start(config.energy, callback);
    });
}
