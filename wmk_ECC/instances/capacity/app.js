...
function run(param) {
    ah.series([print_run_info, function (cb) {
        ...
        if(err){
            ...
            logger.mark('start capacity success');
...
function print_run_info(callback) {
    logger.mark('-----------------------------------');
    logger.mark('start capacity service version %s', version);
...
function init_db(param, callback) {
    ...
    db.init({db: {}, cache: ['no', 'nor', 'oo', 'oor', 'dep', 'role', {name: 'cfg', param: {system: svrType}}]}, function (err) {
        if(err){
...
function init_central_service(callback) {
    ...
    server_manager.init({host: config.central.host, port: config.central.port}, {host: config.capacity.host, port: config.capacity.nport, type: svrType, notifies: [
...
function init_modules(callback) {
    var capacity = require('../../modules/capacity');
    capacity.init(function (err) {
        if(err) return callback(err);

        var http = require('../../interface');
        http.start(config.capacity, callback);
    });
}
