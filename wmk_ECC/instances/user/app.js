...
function print_run_info(callback) {
    logger.mark('-----------------------------------');
    logger.mark('start session service version %s', version);
...
function init_db(param, callback) {
    ...
    db.init({cache: ['no', 'oo', 'oor', 'role', 'privilege', {name: 'cfg', param: {system: svrType}}]}, function (err) {
        if(err){
...
function init_central_service(callback) {
    ...
    server_manager.init({host: config.central.host, port: config.central.port}, {host: config.user.host, port: config.user.nport, type: svrType, notifies: [
...
function init_module(callback) {
    ...
    http.start(config.user, callback);
}
