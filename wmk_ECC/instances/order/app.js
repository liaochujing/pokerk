...
function init_db(param, callback) {
    ...
    db.init({db: {}, cache: ['no', 'nor', 'oo', 'oor', 'cno', 'cnor', 'order', 'role', 'inspection_data', {name: 'cfg', param: {system: svrType}}]}, function (err) {
        if(err){
...
function init_central_service(callback) {
    ...
    server_manager.init({host: config.central.host, port: config.central.port}, {host: config.order.host, port: config.order.nport, type: svrType, notifies: [
...
function init_modules(callback) {
    ...
    order.init(function (err) {
        if(err) return callback(err);

        var http = require('../../interface');
        http.start(config.order, callback);
    });
}
