...
function init_db(param, callback) {
    ...
    db.init({cache: ['no', 'nor', 'oo', 'oor', 'cno', 'cnor', 'dep', 'cdt', 'lc', 'role', {name: 'cfg', param: {system: svrType}}]}, function (err) {
        if(err){
...
function init_central_service(callback) {
    ...
    server_manager.init({host: config.central.host, port: config.central.port}, {host: config.manage.host, port: config.manage.nport, type: svrType, notifies: [
...
function init_module(callback) {
    ...
    manager.init(function (err) {
        ...
        http.start(config.manage, callback);
    });
}
