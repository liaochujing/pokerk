...
function handle(req, res, body, callback) {
    ...
    datadb.rmday.query(match, {id: 1, day: 1, cTemp: 1, hTemp: 1}, {time: 1}, function (err, results) {
        ...
        for(var j=0;j<roomList.length;j++){
            ...
            var roomItem = {
                name: room.fullName,
                curCTemp: data ? data.cTemp || 0 : 0,
                curHTemp: data ? data.hTemp || 0 : 0,
                cTempList: [],
                hTempList: []
            };
            ...
            for(var i in dic){
                var item = dic[i];
                var _temp = item[room.id];
                if(_temp){
                    roomItem.cTempList.push(_temp.cTemp || 0);
                    roomItem.hTempList.push(_temp.hTemp || 0);
                }else{
                    roomItem.cTempList.push(0);
                    roomItem.hTempList.push(0);
                }
            }
...
module.exports.cmd = cmd.cmd_0x00004022;
