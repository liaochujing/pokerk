...
function handle(req, res, body, callback) {
    ...
    datadb.rmday.query(match, {serverNO: 1, id: 1, day: 1, pue: 1}, function (err, datas) {
        ...
        if(datas){
            ...
            for(var i in datas){
                ...
                if(!systemItemDic[item.day][item.serverNO]){
                    systemItemDic[item.day][item.serverNO] = {};
                }
                systemItemDic[item.day][item.serverNO][item.id] = item.pue;
            }
            ...
            for(var i in systemItemDic){
                ...
                for(var j in roomList){
                    ...
                    var value = 0;
                    if(item[room.serverNO]){
                        value = item[room.serverNO][room.id]
                    }

                    sysItem.data.push(value || 0);
                    index++;
                }
            }
...
module.exports.cmd = cmd.cmd_0x00001013;
