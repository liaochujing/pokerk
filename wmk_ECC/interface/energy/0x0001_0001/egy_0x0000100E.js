...
function handle(req, res, body, callback) {
    ...
    var get_project_power_info = function (done) {
        datadb.amday.aggregate([
            {$match: {serverNO: body.serverNO, classID: body.classID, id: body.id, time: {$gt: startTime.AddDays(-3), $lt: startTime}}},
            {$group: {
                _id: '',
                energy: {$last: 'energy'}
            }}
            ...
            if(results && results.length > 0){
                start = results[0].energy;
            }

            var  data = realTimeProcessor.get_area_data_cache(body);
            if(data){
                project_energy = data.energy - start;
            }
    ...
    var get_system_power_info = function (done) {
        datadb.stday.aggregate([
            {$match: {serverNO: body.serverNO, classID: body.classID, id: body.id, time: {$lt: startTime}}},
            {$group: {
                _id: '$system',
                energy: {$last: '$energy'}
            }}
            ...
            for(var i in EnergySystem){
                ...
                var data = realTimeProcessor.get_system_data_cache(body, sys);

                systemPowerData[sys] = (data.energy || 0) - (dic[sys] || 0)
            }
...
module.exports.cmd = cmd.cmd_0x0000100E;
