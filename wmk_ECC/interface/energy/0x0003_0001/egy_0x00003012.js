...
function handle(req, res, body, callback) {
    ...
    if(classID == uiclassID.room){
        _collection = datadb.rmdata;
    }else if(classID >= uiclassID.area && classID <= uiclassID.lastArea){
        _collection = datadb.amdata;
    }else if(classID == uiclassID.cabinet){
        _collection = datadb.cmdata;
    }else{
        _collection = datadb.dmdata;
    }
    var key = body.key;

    var fields = {serverNO: 1, classID: 1, id: 1, time: 1};
    fields[key] = 1;
    _collection.query({$or: body.list, time: {$gte: startTime, $lt: endTime}}, fields, function (err, results) {
        ...
        if(results){
            ...
            for(var i=0;i<results.length;i++){
                ...
                _dic[item.id].list.push({value: item[key] || 0, time: item.time});
            }
        }
...
module.exports.cmd = cmd.cmd_0x00003012;
