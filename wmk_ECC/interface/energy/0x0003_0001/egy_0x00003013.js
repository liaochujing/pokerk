...
function handle(req, res, body, callback) {
    ...
    if(classID == uiclassID.room){
        _collection = datadb.rmday;
    }else if(classID >= uiclassID.area && classID <= uiclassID.lastArea){
        _collection = datadb.amday;
    }else if(classID == uiclassID.cabinet){
        _collection = datadb.cmday;
    }else{
        _collection = datadb.dmday;
    }
    var key = body.key;

    var fields = {serverNO: 1, classID: 1, id: 1, day: 1};
    fields[key] = 1;
    _collection.query({$or: body.list, time: {$gte: startTime, $lt: endTime}}, fields, function (err, results) {
        ...
        if(results){
            ...
            for(var i=0;i<results.length;i++){
                ...
                _dic[item.id].list.push({value: item[key] || 0, day: item.day});
            }
        }
...
module.exports.cmd = cmd.cmd_0x00003013;
