...
function handle(req, res, body, callback) {
    ...
    if(body.serverNO == defaultServerNO){
        alarmHelper.hangup_alarm(body.serverNO, body.classID, body.id, option, function (err) {
            if(err) return callback({status: code.unknown});

            dbAML.insert_log(body.serverNO, body.classID, body.id, user.id, AlarmAction.hangup_alarm, option, new Date(), function (err) {
                if(err) logger.error({msg: 'insert alarm action log failed', user: user, action: AlarmAction.hangup_alarm});
            });

            callback(null, {isHangup: 1, hangup: option})
        })
    }else{
            ...
            dbDP.find_device_point({deviceServerNO: source.serverNO, deviceClassID: source.classID, deviceID: source.id, pointIndex: source.pointIndex}, {_id: 0, fullName: 1, config: 1, binding: 1, option: 1}, function (err, results) {
                ...
                dbNO.update_nameobject_time(source.serverNO, source.classID, source.id, time, function (err) {
                    ...
                    dbDP.update_devicepoint_config(source.serverNO, source.classID, source.id, source.pointIndex, config, time, function (err) {
                        ...
                        dbOPL.insert_user_log(AlarmAction.hangup_point, user, '挂起设备点:' + _point.fullName + ' 告警', {serverNO: source.serverNO, classID: source.classID, id: source.id, pointIndex: source.pointIndex, hangup: hangup}, new Date());
                    });
                });
            });
        };

        var msg = new DataMessage(CentralCMD.cen_0x00020004, body);
        dataHelper.send(msg, function (err, data) {
            if(err){
                callback({status: code.unknown, msg: err.msg});
                return;
            }
...
module.exports.cmd = cmd.alm_0x00000003;
