...
function handle(req, res, body, callback) {
    ...
    if(body.serverNO != undefined){
        ...
        order = objectHelper.createNameObject(body.classID)
    }
    ...
    _gen_order(order, function (err) {
        ...
        if(!body.id){
            dbOPL.insert_user_log(OrderAction.add_order, user, '创建上下电工单:' + order.fullName, req.body, new Date())
        }else{
            dbOPL.insert_user_log(OrderAction.modify_order, user, '修改上下电工单:' + order.fullName, req.body, new Date())
        }
...
module.exports.cmd = cmd.odr_0x00000021;
module.exports.handle = handle;
module.exports.paramSchema = {
    type: 'object',
    properties: {
        serverNO: {type: 'number'},
        classID: {type: 'number', enum: [uiclassID.onPowerOrder, uiclassID.offPowerOrder], required: true},
        id: {type: 'number'},
