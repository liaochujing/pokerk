...
function handle(req, res, body, callback) {
    ...
    if(order_arr && order_arr.length > 0){
        for(var i=order_arr.length-1;i>=0;i--){
            ...
            var _aNO = undefined;
            if(order.restData.asset){
                _aNO = order.restData.asset.aNO;
            }
            if(aNO &&  _aNO.indexOf(aNO) >= 0){
                continue;
            }
            ...
            if(order.state != OrderState.draft){
                ...
                if((order.restData.maintain != undefined && order.restData.maintain == user.id)){//维护人是当前登陆人
                    add_order(order);
                    continue;
                }
            }
...
module.exports.cmd = cmd.odr_0x00010005;
