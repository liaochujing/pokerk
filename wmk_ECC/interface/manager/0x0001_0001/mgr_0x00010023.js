...
function handle(req, res, body, callback) {
    ...
    dbObjID.gen_object_id('link_control', function (err, id) {
        if(err) return callback(err);
        var now = new Date();
        dbLC.insert_link_control(id, body.type, body.name, body.option, body.enabled, now, function (err) {
            if(err) return callback(err);
            ...
            notification.emit(notification.Emitter.link_control, 'add', _lc);
...
module.exports.cmd = cmd.mgr_0x00010023;
