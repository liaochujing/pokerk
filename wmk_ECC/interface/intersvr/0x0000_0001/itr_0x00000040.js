    ...
    var get_instance_state = function (done) {
        switch(body.classID){
            ...
            case uiclassID.building:
                ...
                var devices = [];
                devices.push({serverNO: body.serverNO, classID: body.classID, id: body.id});
            ...
            case uiclassID.room:
                ...
                for(var i=0;i<response.length;i++){
                    var item = response[i];
                    item.online = def.unknownPV;
                    if(item.classID >= uiclassID.collector){
                        devices.push({serverNO: item.serverNO, classID: item.classID, id: item.id});
                    }
                }
                var build = rdbNOR.get_parent_identity(body.serverNO, body.classID, body.id, uiclassID.building);
                if(build){
                    devices.push({serverNO: build.serverNO, classID: build.classID, id: build.id});
                    ...
                    dataHelper.send(msg, function (err, results) {
                        ...
                        if(results && results.length > 0){
                            var builds = results;
                            var isOnline = def.unknownPV;
                            let _dic = {};
                            for(let i=0;i<results.length;i++){
                                let _item = results[i];
                                if(_item.id == build.id){
                                    isOnline = _item.online;
                                }else{
                                    _dic[_item.id] = _item;
                                }
                            }
                            ...
                            for(var i=0;i<response.length;i++){
                                ...
                                if(item.classID >= uiclassID.collector){
                                    let _state = _dic[item.id];
                                    if(_state){
                                        item.online = _state.online;
                                    }
                                }else{
                                    item.online = isOnline;
                                }
...
module.exports.cmd = cmd.itr_0x00000040;
