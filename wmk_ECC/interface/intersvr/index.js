    ...
    if (msg_hander != null) {
        middleware.check_privilege(req, res, msg_hander, function (err) {
            ...
            middleware.check_param(req, res, msg_hander, function (err) {
                ...
                if(req.body.cmd && req.body.name){
                    dbOPL.insert_user_log(1, req.user, '查询:' + req.body.name, null, new Date());
                }
...
router.use('/api/common', require('./router/intersvr'));
router.use('/api/device', require('./router/xh_intersvr'));
