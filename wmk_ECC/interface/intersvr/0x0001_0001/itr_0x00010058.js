...
function handle(req, res, body, callback) {
        ...
        if(area.classID == uiclassID.room){
            ...
            if(_rooms){
                for(var j=0;j<_rooms.length;j++){
                    var room = _rooms[j];
                    rooms.push({serverNO: room.serverNO, areaID: room.id})
                }
            }
        }
    }
    ...
    if(rooms.length > 0){
        db.datadb.rmmon.query({$or: rooms, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, month: 1, outVol: 1}, {time: 1}, function (err, results) {
            ...
            if(results){
                for(var i=0;i<results.length;i++){
                    ...
                    dic[key].push({month: item.month, outVol: item.outVol})
                }
            }
...
module.exports.cmd = cmd.itr_0x00010058;
