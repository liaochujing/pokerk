...
function handle(req, res, body, callback) {
    ...
    for(var i=0;i<body.devices.length;i++){
        var device = rno.get_by_identity(body.devices[i]);
        if(device){
            var _type = rdep.get_device_type(device.type);
            if(_type && _type.pointList){
                for(var j=0;j<_type.pointList.length;j++){
                    var _p = _type.pointList[j];
                    if(_p.busType == 9){//能耗点
                        points.push({serverNO: device.serverNO, classID: device.classID, id: device.id, pointIndex: _p.pointIndex});
                        break;
                    }
                }
            }
    ...
    var msg = new DataMessage(CentralCMD.cen_0x00030003, {points: points, startTime: body.startTime, endTime: body.endTime});
                ...
                if(_list){
                    ...
                    while(_start < endTime){
                        ...
                        if(value != undefined){
                            if(_preKey){
                                if(_preValue){
                                    var _vv = format_value(value - _preValue);
                                    if(_vv >= threshold){
                                        resList.push({day: _preKey, value: _vv})
                                    }
                                }else{
...
module.exports.cmd = cmd.itr_0x00010069;
