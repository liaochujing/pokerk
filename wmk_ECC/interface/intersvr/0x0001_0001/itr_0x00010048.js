...
function handle(req, res, body, callback) {
    ...
    if(areas.length > 0){
        ...
        var get_area_data = function (done) {
            db.datadb.amday.query({$or: areas, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, day: 1, enUsage: 1, itENUsage: 1/*, ljENUsage: 1, slENUsage: 1, dxENUsage: 1*/}, {time: 1}, function (err, results) {
                ...
                if(results){
                    for(var i=0;i<results.length;i++){
                        ...
                        resDic[_key].list.push({day: item.day, energy: item.enUsage, '1_1': item.itENUsage/*, '2_1_1': item.ljENUsage, '2_1_2': item.slENUsage, '2_1_3': item.dxENUsage*/});
                    }
                }
        ...
        var get_system_group_data = function (done) {
            db.datadb.sgday.query({$or: areas, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, system: 1, group: 1, day: 1, enUsage: 1}, {time: 1}, function (err, results) {
                if(err) return done(err);
                ...
                if(results){
                    ...
                    for(var i=0;i<results.length;i++){
                        ...
                        dic[_key][item.day].push({system: item.system, group: item.group, energy: format_value(item.enUsage)});
                    }
...
module.exports.cmd = cmd.itr_0x00010048;
