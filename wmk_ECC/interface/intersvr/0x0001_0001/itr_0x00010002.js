...
function handle(req, res, body, callback) {
    ...
    var response = [];
    var get_current = function (done) {
        db.alarm.find_alarm(query, {_id: 0, serverNO: 1, id: 1, source: 1, level: 1, startTime: 1, endTime: 1, desc: 1, 'start.pointValue': 1}, function (err, alarms) {
            if(err) return done({status: code.unknown, msg: err.message});
    ...
    var get_history = function (done) {
        db.datadb.hisalarm.find_his_alarm(query, {_id: 0, serverNO: 1, id: 1, source: 1, level: 1, startTime: 1, endTime: 1, desc: 1, 'start.pointValue': 1}, function (err, alarms) {
            if(err) return done({status: code.unknown, msg: err.message});
            for(var i=0;i<alarms.length;i++){
                var record = alarms[i];
...
module.exports.cmd = cmd.itr_0x00010002;
