...
function handle(req, res, body, callback) {
    ...
    if(rooms.length > 0){
        db.datadb.rmday.query({$or: rooms, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, day: 1, enUsage: 1}, {time: 1}, function (err, results) {
            ...
            if(results){
                for(var i=0;i<results.length;i++){
                    ...
                    dic[key].push({day: item.day, enUsage: item.enUsage})
                }
            }
            ...
            for(var i=0;i<rooms.length;i++){
                ...
                if(room){
                    ...
                    var roomList = dic[format_key(room.serverNO, room.classID, room.id)];
                    if(roomList){
                        var preUsage = undefined;
                        for(var j=0;j<roomList.length;j++){
                            var row = roomList[j];

                            if(preUsage != undefined){
                                if(Math.abs(row.enUsage - preUsage) >= threshold){
                                    row.preENUsage = preUsage;
                                    roomItem.list.push(row)
                                }
                            }
                            preUsage = row.enUsage;
                        }
                    }
...
module.exports.cmd = cmd.itr_0x00010065;
