...
function handle(req, res, body, callback) {
    ...
    var _time = startTime;
    while(_time < endTime){
        _keyList.push(_time.format_month());
        _time = _time.AddMonth(1);
    }
        ...
        db.datadb.cmmon.query({$or: areas, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, enUsage: 1, month: 1}, {time: 1}, function (err, results) {
            ...
            for(var k in dic){
                ...
                for(var j=0;j<_keyList.length;j++){
                    var month = _keyList[j];
                    cabItem.list.push({month: month, value: _dic[month] || 0})
                }
...
module.exports.cmd = cmd.itr_0x00010063;
