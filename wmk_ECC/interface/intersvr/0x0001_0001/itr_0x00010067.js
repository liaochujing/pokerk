...
function handle(req, res, body, callback) {
    ...
    if(rooms.length > 0){
        db.datadb.rmday.query({$or: rooms, time: {$gte: startTime, $lt: endTime}}, {_id: 0, serverNO: 1, classID: 1, id: 1, day: 1, pue: 1}, {time: 1}, function (err, results) {
            ...
            for(var i=0;i<rooms.length;i++){
                ...
                if(room){
                    ...
                    if(roomList && roomList.length > 0){
                        var prePue = roomList[0].pue;
                        for(var j=1;j<roomList.length;j++){
                            var row = roomList[j];

                            if(Math.abs(row.pue - prePue) >= threshold){
                                row.prePue = prePue;
...
module.exports.cmd = cmd.itr_0x00010067;
